...
function renderInline(text) {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith("`") && part.endsWith("`"))
      return <code key={i} style={{ background: "#e5e7eb", padding: "1px 5px", borderRadius: 3, fontSize: 11.5, fontFamily: "monospace" }}>{part.slice(1, -1)}</code>;
    return part;
  });
}
  ...
  while (i < lines.length) {
    ...
    if (line.trim().startsWith("|")) {
      ...
      if (rows.length > 0) {
        ...
        out.push(
          <table key={out.length} style={{ borderCollapse: "collapse", width: "100%", margin: "8px 0", fontSize: 12 }}>
            <thead>
              <tr>{parseRow(header).map((cell, ci) => (
                <th key={ci} style={{ border: "1px solid #e5e7eb", padding: "6px 10px", background: "#f3f4f6", fontWeight: 600, textAlign: "left" }}>{renderInline(cell)}</th>
              ))}</tr>
            </thead>
            <tbody>
              {body.map((row, ri) => (
                <tr key={ri}>{parseRow(row).map((cell, ci) => (
                  <td key={ci} style={{ border: "1px solid #e5e7eb", padding: "5px 10px" }}>{renderInline(cell)}</td>
                ))}</tr>
              ))}
            </tbody>
          </table>
    ...
    // Headings
    if (/^### /.test(line)) {
      out.push(<div key={out.length} style={{ fontSize: 13, fontWeight: 600, color: "#111827", margin: "12px 0 4px" }}>{renderInline(line.slice(4))}</div>);
      i++; continue;
    }
    if (/^## /.test(line)) {
      out.push(<div key={out.length} style={{ fontSize: 13.5, fontWeight: 600, color: "#111827", margin: "14px 0 5px", borderBottom: "1px solid #f3f4f6", paddingBottom: 3 }}>{renderInline(line.slice(3))}</div>);
      i++; continue;
    }
    if (/^# /.test(line)) {
      out.push(<div key={out.length} style={{ fontSize: 14, fontWeight: 700, color: "#111827", margin: "14px 0 6px" }}>{renderInline(line.slice(2))}</div>);
      i++; continue;
    }
    ...
    if (/^[-*] /.test(line)) {
      ...
      out.push(
        <ul key={out.length} style={{ margin: "4px 0 8px", paddingLeft: 18 }}>
          {items.map((item, ii) => <li key={ii} style={{ marginBottom: 3, lineHeight: 1.65 }}>{renderInline(item)}</li>)}
        </ul>
      );
      continue;
    }
    ...
    if (/^\d+\. /.test(line)) {
      ...
      out.push(
        <ol key={out.length} style={{ margin: "4px 0 8px", paddingLeft: 18 }}>
          {items.map((item, ii) => <li key={ii} style={{ marginBottom: 3, lineHeight: 1.65 }}>{renderInline(item)}</li>)}
        </ol>
      );
      continue;
    ...
    // Blockquote
    if (/^> /.test(line)) {
      out.push(<div key={out.length} style={{ borderLeft: "3px solid #d1d5db", paddingLeft: 10, color: "#6b7280", margin: "4px 0 8px", fontStyle: "italic" }}>{renderInline(line.slice(2))}</div>);
      i++; continue;
    }

    // Regular paragraph
    out.push(<p key={out.length} style={{ margin: "0 0 8px", lineHeight: 1.75 }}>{renderInline(line)}</p>);
    i++;
  }
  return out;
}
  ...
  return (
    <div style={{ display: "flex", justifyContent: u ? "flex-end" : "flex-start", marginBottom: 12 }}>
      <div style={{
        ...
        {u ? msg.content : renderMarkdown(msg.content)}
      </div>
    </div>
  );
}
